...
const Guarniciones = (props) => {
  ...
  const obtenerGuarniciones = async () => {
    const dia = props.route.params.dia;
    const response = await axios.get(baseUrl + 'obtenerGuarniciones/' + dia);
    const { data } = response;
  ...
  return (
    <Block flex center style={styles.guarniciones}>
      <ScrollView
        ...
        <SafeAreaView>
          {comidas.length != 0 ? (
            ...
            <Block style={styles.containerVacio}>
              <Text style={styles.containerVacioText}>Sin guarniciones!</Text>
            </Block>
          )}
        </SafeAreaView>
      </ScrollView>
