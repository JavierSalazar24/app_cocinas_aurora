...
const Sopas = (props) => {
  ...
  const obtenerSopas = async () => {
    const dia = props.route.params.dia;
    const response = await axios.get(baseUrl + 'obtenerSopas/' + dia);
    const { data } = response;
  ...
  return (
    <Block flex center style={styles.sopas}>
      <ScrollView
        ...
        <SafeAreaView>
          {comidas.length != 0 ? (
            ...
            <Block style={styles.containerVacio}>
              <Text style={styles.containerVacioText}>Sin sopas!</Text>
            </Block>
          )}
        </SafeAreaView>
      </ScrollView>
